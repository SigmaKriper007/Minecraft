...
public final class EmberChargeLayer<T extends Player, M extends HumanoidModel<T>> extends RenderLayer<T, M> {
    private static final ResourceLocation TEXTURE = new ResourceLocation("opusvsexe:textures/ember/entity/fireball.png");
    private static int totalTicks = 16;
    private static int ticksLeft;

    public EmberChargeLayer(RenderLayerParent<T, M> parent) { super(parent); }
    ...
    private static void vertex(PoseStack.Pose pose, VertexConsumer out, float x,float y,float z,float u,float v) {
        out.vertex(pose.pose(),x,y,z).color(255,154,31,220).uv(u,v).overlayCoords(OverlayTexture.NO_OVERLAY)
