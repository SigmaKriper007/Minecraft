...
public final class LavaGolemEntity extends Monster implements GeoAnimatable {
    ...
    public LavaGolemEntity(EntityType<? extends Monster> type, Level level) {
        super(type, level);
        xpReward = 18;
    }
    ...
    public void tick() {
        ...
        if (level() instanceof ServerLevel server && tickCount % 8 == 0) {
            server.sendParticles(FireParticles.EMBER, getX(), getY() + 1.4, getZ(), 2, 0.45, 0.8, 0.45, 0.01);
        }
    }
    ...
    public boolean doHurtTarget(Entity target) {
        ...
        if (hit && target instanceof LivingEntity living) {
            ...
            level().playSound(null, this, FireSounds.GOLEM_ATTACK, SoundSource.HOSTILE, 1.3f, 0.82f + random.nextFloat() * 0.12f);
        }
        return hit;
    }
    ...
    public boolean hurt(DamageSource source, float amount) {
        ...
        if (hit && !level().isClientSide) {
            level().playSound(null, this, FireSounds.GOLEM_HURT, SoundSource.HOSTILE, 1.0f, 0.78f + random.nextFloat() * 0.16f);
        }
        return hit;
    }
    ...
    private PlayState selectAnimation(AnimationState<LavaGolemEntity> state) {
