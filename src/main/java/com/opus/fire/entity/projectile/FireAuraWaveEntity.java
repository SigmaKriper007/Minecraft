...
/** Expanding three-dimensional pulse emitted by the Sovereign of Four Veins. */
public class FireAuraWaveEntity extends Entity implements GeoAnimatable {
    ...
    public FireAuraWaveEntity(EntityType<? extends Entity> type, Level level) {
        super(type, level);
        this.setNoGravity(true);
    }

    public FireAuraWaveEntity(Level level, LivingEntity owner) {
        super(FireEntities.FIRE_AURA_WAVE, level);
        this.setNoGravity(true);
        this.owner = owner;
    }
    ...
    public void tick() {
        ...
        if (this.tickCount == 1) {
            this.level().playSound(null, this.getX(), this.getY(), this.getZ(),
                FireSounds.DEMON_AURA, SoundSource.HOSTILE, 1.5f, 1.0f);
        }
        // Урон по мере расширения кольца (урон при контакте, без повторов)
        this.dealDamage();
