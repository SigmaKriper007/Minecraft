...
public final class BlazingTridentEntity extends Projectile implements GeoAnimatable {
    private static final EntityDataAccessor<Boolean> RETURNING = SynchedEntityData.defineId(BlazingTridentEntity.class, EntityDataSerializers.BOOLEAN);
    ...
    public BlazingTridentEntity(EntityType<? extends Projectile> type, Level level) { super(type, level); }

    public BlazingTridentEntity(EntityType<? extends Projectile> type, Level level, LivingEntity owner) {
        this(type, level);
        setOwner(owner);
    ...
    public void tick() {
        ...
        if (level().isClientSide) {
            level().addParticle(EmberParticles.EMBER_SPARK, getX() - velocity.x * 0.25, getY() - velocity.y * 0.25,
    ...
    private void impact(HitResult hit) {
        ...
        if (level() instanceof ServerLevel server) {
            ProtectedEmberExplosion.explode(server, this, getOwner(), 4.0f);
            server.sendParticles(EmberParticles.EMBER_SPARK, getX(), getY(), getZ(), 28, 0.8, 0.8, 0.8, 0.2);
            server.playSound(null, this, EmberSounds.EMBER_EXPLODE, SoundSource.HOSTILE, 1.7f, 0.76f);
        }
        beginReturn();
    }
